...
class SettingsWindow(QMainWindow):
    def __init__(self):
        ...
        self.saveCode.clicked.connect(self.save_code)
...
class TryImgWindow(QMainWindow):
    def __init__(self):
        ...
        self.go_default()  # Устанавливаем изображение по умолчанию
...
class TryDBWindow(QMainWindow):
    def __init__(self):
        ...
        # Привязываем к кнопкам действия
        self.chooseDB.addItems(["10 А", "10 Б"])
        self.chooseDB.activated[str].connect(self.choose_class)
        ...
        self.choose_class('10 А')

        self.update_table()

    def choose_class(self, selected):
        if selected == "10 А":
            self.classname = '[10_a]'
            self.update_table()
        else:
    ...
    def create_str(self):
        self.con.exec_(f"""
        INSERT INTO {self.classname} (name)
    ...
    def update_str(self):
        self.con.exec_(f"""
        UPDATE {self.classname} SET name='{self.updSndInp.toPlainText()}' WHERE name='{self.updFstInp.toPlainText()}'
        """)
        self.update_table()
    ...
    def update_table(self):
        ...
        query = QSqlQuery(f"SELECT name FROM {self.classname}")
        while query.next():
